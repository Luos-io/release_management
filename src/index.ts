import * as core from '@actions/core';
import * as github from '@actions/github';
import * as fs from 'fs';
import semver from 'semver';

function retrieveSourceBranchVersion() {
  // const tmpCtx = {
  //   context: {
  //     payload: {
  //       head_ref: 'refs/heads/release/1.0.0',
  //       pull_request: {
  //         head: { ref: 'release/1.0.0' },
  //         base: { ref: 'main' },
  //         merged: true,
  //       },
  //     },
  //     eventName: 'push',
  //     sha: '',
  //     ref: '',
  //     workflow: 'publish.yml',
  //     action: 'hello',
  //     actor: 'nektos/act',
  //     job: 'Publish',
  //     runNumber: 1,
  //     runId: 1,
  //     apiUrl: 'https://api.github.com',
  //     serverUrl: 'https://github.com',
  //     graphqlUrl: 'https://api.github.com/graphql',
  //   },
  // };

  const sourceBranchName = github.context.payload?.pull_request?.head?.ref;
  if (sourceBranchName) {
    const matchPattern = sourceBranchName.match(/^release\/(\d+\.\d+\.\d+)$/);
    if (matchPattern && matchPattern.length >= 2) {
      return semver.valid(matchPattern[1]);
    }
  }

  return null;
}

function retrievePackageVersion() {
  try {
    return semver.valid(
      JSON.parse(fs.readFileSync('package.json', 'utf8')).version,
    );
  } catch (err) {
    return null;
  }
}

try {
  // TMP
  core.info(JSON.stringify(core));
  core.info(JSON.stringify(github));
  // END TMP
  core.debug(`Starting 'Release Management' action...`);

  const version = retrievePackageVersion();
  if (!version) {
    core.setFailed('Could not read package.json');
  } else {
    core.debug(`Current version: ${version}`);

    const sourceBranchVersion = retrieveSourceBranchVersion();
    if (!sourceBranchVersion) {
      core.warning('Could not retrieve source branch version');
      const type = core.getInput('type');
      switch (type) {
        case 'major':
        case 'minor':
        case 'patch':
          core.setOutput('version', semver.inc(version, type));
          break;
        case 'rc':
          core.setOutput('version', semver.inc(version, 'prerelease', 'rc'));
          break;
        default:
          core.setFailed(
            `Unknown type: '${type}' (expected: 'major', 'minor', 'patch')`,
          );
      }
    } else {
      core.debug(`Source branch version: ${sourceBranchVersion}`);
      if (semver.lt(sourceBranchVersion, version)) {
        core.setFailed('New version is lower than package version');
      } else {
        core.setOutput('version', sourceBranchVersion);
      }
    }
  }
} catch (err) {
  core.setFailed((err as Error).message);
}
